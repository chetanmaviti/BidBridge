/**
 * Pure, client-safe geographic utilities.
 *
 * No I/O, no node:* imports — safe to bundle into client components.
 * The server-only `geocode()` function (which calls Nominatim) lives
 * in lib/geocode.ts and pulls in lib/cache.ts.
 */
...
/**
 * Approximate state-centroid coordinates for fallback distance scoring
 * when an opportunity has only state, no city.
 */
...
/**
 * Crude region grouping for similarity scoring fallback.
 */
export function sameRegion(a: string, b: string): boolean {
  ...
  for (const states of Object.values(REGIONS)) {
    if (states.includes(a) && states.includes(b)) return true;
  }
  return false;
}
